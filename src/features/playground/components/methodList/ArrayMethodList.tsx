...
const CATEGORY_ORDER: ArrayMethodCategory[] = ['mutator', 'accessor', 'iteration', 'search', 'static'];

type ArrayMethodListProps = {
  methods: ArrayMethodDefinition[];
  selectedId: string;
  onSelect: (id: string) => void;
  categoryFilter: ArrayMethodCategory | 'all';
  onCategoryChange: (cat: ArrayMethodCategory | 'all') => void;
  methodSearch: string;
  onMethodSearchChange: (q: string) => void;
  categoryCounts: Record<ArrayMethodCategory | 'all', number>;
  className?: string;
  onSelectAndClose?: () => void;
};

export function ArrayMethodList({
  methods,
  selectedId,
  onSelect,
  categoryFilter,
  ...
  return (
    <div className={cn('flex flex-col gap-3 min-h-0', className)}>
      <Tabs
        ...
        onValueChange={(v) => onCategoryChange(v as ArrayMethodCategory | 'all')}
        className="w-full"
      >
          ...
          >
            ...
            {CATEGORY_ORDER.map((cat) => (
              <TabsTrigger key={cat} value={cat} className="text-xs px-2.5 py-1.5">
                {ARRAY_METHOD_CATEGORIES[cat].label}
                <Badge variant="secondary" className="ml-1 h-4 px-1 text-[10px]">
                  {categoryCounts[cat]}
                </Badge>
              </TabsTrigger>
          ...
          {methods.map((method) => (
            ...
            >
              ...
              <span className="block text-[10px] uppercase tracking-wide opacity-70 mt-0.5">
                {ARRAY_METHOD_CATEGORIES[method.category].label}
              </span>
            </button>
          ))}
          {methods.length === 0 && (
